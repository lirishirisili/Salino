...
function removeCommonHebrewPrefixes(s: string): string {
  let r = s;
  if (r.startsWith('ה')) r = r.slice(1);
  if (r.startsWith('ו')) r = r.slice(1);
  if (r.startsWith('ב')) r = r.slice(1);
  if (r.startsWith('ל')) r = r.slice(1);
  return r;
}

function removeCommonHebrewSuffixes(s: string): string {
  let r = s;
  if (r.endsWith('ים') && r.length > 2) r = r.slice(0, -2);
  else if (r.endsWith('ות') && r.length > 2) r = r.slice(0, -2);
  else if (r.endsWith('ה') && r.length > 1) r = r.slice(0, -1);
  return r;
}

function removeEnglishPluralSuffix(s: string): string {
  if (s.endsWith('es') && s.length > 4) return s.slice(0, -2);
  if (s.endsWith('s') && s.length > 3) return s.slice(0, -1);
  return s;
}

function hebrewConstructToBase(s: string): string {
  if (s.endsWith('\u05EA') && s.length > 2) return s.slice(0, -1) + '\u05D4';
  return s;
}

function removeArabicDefiniteArticle(s: string): string {
...
function tokenVariants(token: string): string[] {
  ...
  variants.add(removeEnglishPluralSuffix(cleaned));
  variants.add(removeArabicDefiniteArticle(cleaned));
...
const keywordMap: Record<ItemCategory, string[]> = {
  DAIRY: [
  ...
  CLEANING: [
    ...
    'jabón', 'detergente', 'lejía', 'esponja', 'limpiador', 'bolsa de basura', 'lavavajillas', 'suavizante', 'toallitas', 'papel higiénico', 'escoba',
